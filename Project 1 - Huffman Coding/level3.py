...
def Huffman_Encoding3(data, total_element):
...
def Huffman_Decoding3(encodedData, huffmanTree, filename, arr):
...
def saveDecompressedFile3(decodedOutput, arr):
...
def readPILimg3(fname):
    ...
    diff_arr = difference(arr)


    return diff_arr, total_element, arr
...
def statistics3(huffman_encoding, problist, data):
...
def difference(arr):
    ...
    for i in range(nrows):
        for j in range(1, ncols):
            darr[i][j] = int(arr[i][j]) - int(arr[i][j - 1])
